...
export class ManageEmailComponent implements OnInit {
  ...
  @ViewChild('senttableInput') senttableInput!: ElementRef;
  @ViewChild('tableInput1') tableInput1!: ElementRef;
  EmailGroupList: any = [];
  ...
  selectSendTab(id: any) {  
    ...
    this.manageEmailList = this.emailList.filter((x: any) => x.status == 2 || x.status == 5)
    this.SystemNotificationList = this.NotificationList.filter((x: any) => x.status == 2)
  ...
  selectArchiveTab(id: any) {
    ...
    this.SystemNotificationList = this.manageEmailList.filter((x: any) => x.status == 2)
  }
  ...
  MailPopupRefresh() {
    var id = this.ValueId
    this._manageEmail.getSendEmailDetails(id).subscribe(data => {
      if (data) {
        let a: any = {};
        a = data;
        this.credorEmailDetail = a.credorEmailDetail;
  ...
  SentEmailListExport() {
    const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(this.senttableInput.nativeElement);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'SentEmail.xlsx');
  }

  ExcelTableExport1() {
    const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(this.tableInput1.nativeElement);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'Campaign.xlsx');
  }
  ...
  getBounceEmail(){
    ...
    this._manageEmail.getBounceEmail().subscribe(data => {
      if(data){
        this.manageEmailList = data;
      }
    })
  }
