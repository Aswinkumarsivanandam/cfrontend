...
export class EmailComponent implements OnInit {
  ...
  smtpHostError: boolean = false;
  autofocus: boolean = false;
  autofocusPassword: boolean = false;
  autofocusSMTP: boolean = false;
  ...
  constructor(private _email: EmailService, private toastr: ToastrService, private route: ActivatedRoute, private router: Router,) { }

  ngOnInit(): void {
    this.TabManageId = 0;
    this.Selected = 'sendEmail';
    this.selectSendEmailTab(1);
    this.adminUserId = Number(localStorage.getItem('UserId'));
    this.TabManageId = +this.route.snapshot.params['id'];
    console.log("tagManageId",this.TabManageId);
    if(this.TabManageId == 3)
    {
      this.selectmanageTemplateTab(3);
    }
  }

  selectVerifyDomainTab(id: any) {
    this.Selected = 'addDomain';
    this.TabId = id;
    this.getDomain();
    this.getEmail();
  }
  ...
  onSMTPHost() {
    if (this.smtpHost == '' || this.smtpHost == null) {
      this.smtpHostError = true;
    }
    else {
      this.smtpHostError = false;
    }
  }
  ...
  saveDomain() {
    ...
    else if (this.smtpHost == '' || this.smtpHost == null || this.smtpHostError == true) {
      this.autofocusSMTP = true;
    }
    else {
      let domain: any = {}
      domain.AdminUserId = this.adminUserId;
      domain.Name = this.domainName;
      domain.SMTPHost = this.smtpHost;
  ...
  saveEmail() {
    ...
    else if (this.Password == '' || this.Password == true || this.Password == null) {
  ...
  selectSendEmailTab(id: any) {
    this.Selected = 'sendEmail';
    this.TabId = id;
    console.log("tabmanageID",this.TabManageId);
    if(this.TabManageId == 3)
    {
      this.TabManageId == 0;
