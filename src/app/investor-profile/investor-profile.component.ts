  ...
  onDeleteProfileConfirmation() {
    ...
    this.profileService.DeleteProfile(this.UserId, this.ProfileId).subscribe(data => {
      if (data == 1) {
        ...
        this.DeletePopup = false;
        this.toastr.info('Profile already mapped with some investment, you cannot be deleted', 'Info!')
        this.Loader = false;
      }
      else {
  ...
  onDeleteBankConfirmation() {
    ...
    this.profileService.DeleteBank(this.UserId, this.BankDeleteId).subscribe(data => {
      if (data == 1) {
        ...
        this.DeleteBankPopup = false;
        this.toastr.info('Bank account mapped with some investment, so you cannot be delete', 'Info!')
        this.Loader = false;
      }
      else {
