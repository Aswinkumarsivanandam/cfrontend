...
export class MyinvestmentComponent implements OnInit {
  ...
  UpdateValue: any = [];
  DistributionHistoryPopup : boolean = false;
  DistributionHistory: any;
  DistributionHistoryDistribution: any = [];
  ...
  GetInvestmentByUserId() {
    this.investmentService.GetInvestmentByUser(this.UserId).subscribe(data => {
      ...
      this.ActiveInvestor = this.InvestmentData.filter((x: { status: number; offeringStatus : number; isReservation : boolean}) => x.status == 1 && (x.offeringStatus == 2 || x.offeringStatus == 3) && x.isReservation == false);
      this.PendingInvestor = this.InvestmentData.filter((x: { status: number; offeringStatus : number; isReservation : boolean}) => x.status == 2 && (x.offeringStatus == 2 || x.offeringStatus == 3) && x.isReservation == false);
      this.PastInvestor = this.InvestmentData.filter((x: {status : number; offeringStatus: number; isReservation : boolean}) =>(x.status == 1 || x.status == 2) && x.offeringStatus == 4 && x.isReservation == false);
  ...
  onReserve(val: any) {
    ...
    if (this.InvestError == false && this.InsAmtError == false && this.MinimumError == false) {
      let reserve = {
        ...
        OfferingId: this.AddEditData.offeringId,
  ...
  onDistributionHistory(val : any){
    this.DistributionHistoryPopup = true;
    this.investmentService.GetDistributionHistory(val.id).subscribe(data =>{
      this.DistributionHistory = data;
      this.DistributionHistoryDistribution = this.DistributionHistory.distributions
    })
  }
}
